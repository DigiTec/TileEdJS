...
Object.defineProperties(TMXObject.prototype, {
  ...
  importObject: {
    value: function(objectNode) {
      ...
      // coordinates.
      this.absoluteX = parseInt(
        objectNode.attributes.getNamedItem("x").nodeValue
      );
      this.absoluteY = parseInt(
        objectNode.attributes.getNamedItem("y").nodeValue
      );

      if (objectNode.hasAttribute("width")) {
        this.absoluteWidth = parseInt(
          objectNode.attributes.getNamedItem("width").nodeValue
        );
      }
      if (objectNode.hasAttribute("height")) {
        this.absoluteHeight = parseInt(
          objectNode.attributes.getNamedItem("height").nodeValue
        );
      }
      if (objectNode.hasAttribute("gid")) {
        this.tileId = parseInt(
          objectNode.attributes.getNamedItem("gid").nodeValue
        );
      }

      [].forEach.call(
        objectNode.childNodes,
        function(childNode) {
