    ...
    value: function(tileSetNode) {
      if (tileSetNode.hasAttribute("source")) {
        throw "TSX files such as " +
          tileSetNode.attributes.getNamedItem("source").nodeValue +
          " are not supported";
      }

      var localAttributes = tileSetNode.attributes;
      this._firstGid = parseInt(
        localAttributes.getNamedItem("firstgid").nodeValue
      );
      this._debugName = localAttributes.getNamedItem("name").nodeValue;
      this._tileWidth = parseInt(
        localAttributes.getNamedItem("tilewidth").nodeValue
      );
      this._tileHeight = parseInt(
        localAttributes.getNamedItem("tileheight").nodeValue
      );

      [].forEach.call(
        tileSetNode.childNodes,
        function(childNode) {
          if (childNode.nodeType == Node.ELEMENT_NODE) {
            switch (childNode.localName) {
              case "image":
                this._imageSource = this._map.mapTileSetSourceToUrl(
                  childNode.attributes.getNamedItem("source").nodeValue
                );
                this._imageWidth = parseInt(
                  childNode.attributes.getNamedItem("width").nodeValue
                );
                this._imageHeight = parseInt(
                  childNode.attributes.getNamedItem("height").nodeValue
                );
                this._cellColumns = this._imageWidth / this._tileWidth;
                this._cellRows = this._imageHeight / this._tileHeight;
                this._lastGid =
                  this._firstGid + this._cellColumns * this._cellRows - 1;
                break;

              case "tile":
                var localId = parseInt(
                  childNode.attributes.getNamedItem("id").nodeValue
                );
                [].forEach.call(
                  ...
                  function(childNode) {
                    if (childNode.nodeType == Node.ELEMENT_NODE) {
                      switch (childNode.localName) {
                        case "properties":
                          this._tileProperties[localId] = new TMXPropertyMap(
                            this
                          );
                          this._tileProperties[localId].importProperties(
                            childNode
                          );
                          break;
