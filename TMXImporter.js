...
Object.defineProperties(TMXImporter.prototype, {
  loadFromUrl: {
    value: function (url, callback) {
      ...
      var callbackObject = { req: request, tmxImporter: this, callback: callback };

      request.open("GET", url, true);
      request.addEventListener("readystatechange", this.loadFromURLCallback.bind(callbackObject));
      request.send(null);
    },
  },
  ...
  loadFromXMLHttpResponse: {
    value: function (request) {
      if (request.readyState != 4) {
        throw "Invalid readyState value for operation: readyState = " + request.readyState;
      }
