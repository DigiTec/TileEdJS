...
Object.defineProperties(TMXMap.prototype, {
  ...
  importMap: {
    value: function (mapNode) {
      if (this.isSupported(mapNode)) {
        ...
        [].forEach.call(mapNode.childNodes, function (childNode) {
  ...
  parseMapData: {
    value: function (mapNode) {
      ...
      this.tileWidth = parseInt(mapNode.attributes.getNamedItem("tilewidth").nodeValue);
      this.tileHeight = parseInt(mapNode.attributes.getNamedItem("tileheight").nodeValue);
    }
  },
    ...
    value: function (gid) {
      for (var i = 0; i < this._tileSets.length; i++) {
        if (this._tileSets[i].containsTile(gid)) {
          var renderData = this._tileSets[i].getTileRenderData(gid);
          if (renderData) {
            return this._tileSets[i].cssUrl + " " + (-renderData.left) + "px " + (-renderData.top) + "px";
          }
          break;
        }
      }
